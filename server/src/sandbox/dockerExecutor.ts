...
export class DockerExecutor {
  async run(options: RunOptions): Promise<string> {
    ...
    const filename =
      languageExtensions[options.language.toLowerCase()] || "index.ts";
    ...
    logger.info(
      { containerId },
      "[DockerExecutor] Container successfully deployed",
    );
  ...
  async exec(
    ...
    try {
      logger.info(
        { containerId, command },
        "[DockerExecutor] Executing command",
      );

      const { stdout, stderr } = await execAsync(
        `docker exec ${containerId} sh -c "${command}; echo EXIT_CODE:$?"`,
        { timeout: 30_000 },
      );
  ...
  async cleanup(containerId: string): Promise<void> {
    try {
      ...
      logger.info(
        { containerId },
        "[DockerExecutor] Container resource wrapper recycled",
      );
    } catch (error) {
      logger.warn(
        { error },
        "[DockerExecutor] Cleanup wrapper completed with warnings",
      );
    }
  }
}
