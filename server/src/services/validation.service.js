...
export default class ValidationService {
    ...
    async validateFix(request) {
                ...
                failures: [
                    {
                        ...
                        message: "Fix agent returned explanatory text instead of executable source code.",
                    },
                ],
            };
        }
        ...
        try {
            const runCommand = this.universalRunner.getExecutionCommand(request.language);
            logger.info({
                language: request.language,
                runCommand,
            }, "[ValidationService] Executing code");
            const executionObj = await this.dockerExecutor.exec(containerId, runCommand);
            logger.info({
